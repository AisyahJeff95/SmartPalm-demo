...
Compiles a standalone, high-end HTML dashboard centered on Seraya Estate, Sabah,
...
# Coordinates of Seraya Estate, Sabah
DEFAULT_LAT = 4.55375
DEFAULT_LON = 117.58288
DEFAULT_PLACE = "Seraya Estate, Sabah"
...
def build_html(
    ...
    geojson_path = Path("seraya_perimeter.geojson")
    ...
    perak_path = Path("perak_perimeter.geojson")
...
<body>
        ...
        <div>
          <div class="sidebar-section-title" style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            ...
            <select id="site-selector" onchange="switchSite(this.value)" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: #fff; font-size: 11px; padding: 2px 6px; border-radius: 4px; outline: none; cursor: pointer; font-family: inherit;">
              <option value="perak">Perak Site</option>
              <option value="seraya" selected>Seraya Estate</option>
            </select>
          </div>
          <div class="glass-card">
            ...
            <div class="meta-row">
              ...
              <span class="meta-val" id="site-area">3,593.42 ha</span>
            </div>
            <div class="meta-row">
  ...
  <script>
    ...
    const map = L.map('map', { zoomControl: false, maxZoom: 18 }).setView([centerLat, centerLon], 13);
    ...
    function getLocalFeatures(lat, lon) {
      ...
      const riverLat = 4.520 + Math.sin((lon - 117.580) * 100) * 0.005;
      const isWater = Math.abs(lat - riverLat) < 0.0006;
      
      // Main Road and secondary roads grid
      const isMainRoad = Math.abs(lon - 117.580) < 0.0002 || Math.abs(lat - 4.540) < 0.00015;
      const dLat = Math.abs(lat - Math.round(lat / 0.0035) * 0.0035);
      const dLon = Math.abs(lon - Math.round(lon / 0.0055) * 0.0055);
      const isSecondaryRoad = (dLat < 0.00005) || (dLon < 0.00005);
      const isRoad = isMainRoad || isSecondaryRoad;

      // HQ building cluster near the main intersection
      const distToHQ = Math.max(Math.abs(lat - 4.540), Math.abs(lon - 117.580));
      const isBuilding = (distToHQ < 0.001) && !isRoad && !isWater && (hash(Math.floor(lat * 1800), Math.floor(lon * 1800)) > 0.45);

      // Bare Soil clearings (patches using high-threshold noise)
      const isBareSoil = (f > 0.76) && !isWater && !isRoad && !isBuilding;

      // Forest buffer zones around river or native forest patches
      const isForestBuffer = Math.abs(lat - riverLat) < 0.0015 && Math.abs(lat - riverLat) >= 0.0006;
      const isForestBufferOutside = !isInsidePerimeter(lat, lon) && (f > 0.35);
    ...
    function generateRasterOverlay(nutrient) {
      // 1. Get bounding box of the Seraya estate perimeter
      let latMin = 90, latMax = -90, lonMin = 180, lonMax = -180;
      serayaPerimeter.forEach(p => {
    ...
    map.on('click', function(e) {
      ...
      if (inside) {
        classification = "Inside Site (" + classification + ")";
      } else {
    ...
    function downloadReport() {
      ...
      csvContent += `Selected Fertilizer,${isPalmZone ? fert.name : "N/A"}\\n`;
      csvContent += `Dosage per Palm (kg),${isPalmZone ? dosagePerPalm.toFixed(2) : "N/A"}\\n`;
      csvContent += `Requirement per Block (MT),${isPalmZone ? reqPerBlockMT.toFixed(5) : "N/A"}\\n\\n`;
      ...
      nutrients.forEach(nut => {
        ...
        csvContent += `${nut.key},${actualText},${targetText},${nut.pct.toFixed(1)}%,${correctivePalm.toFixed(4)},${correctiveBlock.toFixed(2)}\\n`;
      });
    ...
    const fitPoly = L.polygon(serayaPerimeter);
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Palmnex Crop Nutrient interface.")
    ...
    parser.add_argument("--output", type=str, default="palmnex.html", help="Output file.")
